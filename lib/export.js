...
async function exportPlaylistTracks (playlistId) {
  const library = await getLibrary()
  const result = await library.playlistTracks(playlistId)
  return result.tracks
}

async function exportPlaylists (size) {
  const library = await getLibrary()
  const result = await library.playlists({size})
  return Promise.map(result.playlists, async (playlist) => (Object.assign({}, playlist, {
    tracks: await exportPlaylistTracks(playlist.id),
...
async function init () {
  ...
  switch (type) {
    ...
    case 'artists':
      ...
      break
    case 'playlists':
      exportedData = await exportPlaylists(limit)
      break
    default:
